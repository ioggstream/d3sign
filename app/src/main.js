...
// The D3FEND release, generated by app/scripts/rebuild-data.sh from the same
// D3FEND_VERSION that downloads the ontology and rebuilds every projection under
// src/data/ (app/config/README.md). Imported rather than written into index.html so
// the header cannot drift from the data the app answers with.
import d3fendConfig from '../config/d3fend.json';

const DEFAULT_DIAGRAM_ID = 'current';
const UNION_FILTER_KEY = '__union__';

// `testcase*.md` states the supported syntax and the quads it must produce —
// a specification the test suite reads off disk, not a diagram to open. The
// picker offers examples only (ADR 0010), so the negation lives here beside
// the pattern that would otherwise pick them up.
const exampleModules = import.meta.glob(['./data/examples/*.md', '!./data/examples/testcase*.md'], {
  query: '?raw',
  import: 'default',
  eager: true,
});
...
const d3fendVersionLink = document.getElementById('d3fend-version');
d3fendVersionLink.textContent = `D3FEND ${d3fendConfig.version}`;
d3fendVersionLink.href = d3fendConfig.homepage;
const released = d3fendConfig.releaseDate ? `, released ${d3fendConfig.releaseDate}` : '';
d3fendVersionLink.title =
  `Built against D3FEND ${d3fendConfig.version}${released} — the ontology behind ` +
  'completion, node colour and every query answer';
...
async function handleTextChange(text) {
  ...
  const { diagrams: expandedDiagrams, warnings: templateWarnings } = expandDocument(newDiagrams);
  diagrams = expandedDiagrams;
...
function renderSelectedPreview() {
  ...
  // The expanded source when the diagram instantiated a template, so the preview
  // draws the instance the graph draws. It is derived, never written back to the
